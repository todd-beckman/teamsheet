...
/**
 * Fixed forme-name aliases applied to both the display and lookup name. Base
 ...
 */
export function canonicalForme(name: string): string {
...
export function normalizeName(rawNameBeforeAt: string): NormalizedName {
  ...
  // 2. If still `Outer (Inner)`, keep only `Inner` (the forme name; the nickname
  //    wraps the real forme name in parens).
  ...
  // "X Male" / "X Female" keys. Apply fixed forme-name aliases (e.g.
  // Floette/Floette-Mega -> Floette-Eternal) last.
  const lookupName =
    keptSuffixes.length > 0
      ? [baseName, ...keptSuffixes].join("-")
      : baseName;

  const canonical = canonicalForme(lookupName);
